...
for counter=1:length(switch_over_var)
    ...
    switch which_var
        ...
        case 1
            ...
            supfig_name = 'Wuhan: equal G.I.'
            figure_name = 'Figure_Sn_Wuhan_equalGI';

            
        case 2
            
            infile = 'SEIR_agedep_twodiseases_variationsymptomaticity_Wuhan_T5and8.mat';
            load(strcat('./sim_data/',infile));
            ...
            supfig_name = 'Wuhan: different G.I.'
            figure_name = 'Figure_Sn_Wuhan_diffGI';
    ...
    for count=1:8
        ...
        this_h(count) = semilogy(params.t_span, results.y_traj(:,count)/data_Wuhan_Davies.age_distribution(count),'Color',colors_rgb_vector(count,:),'LineWidth',2); hold on;
    ...
    axis([0 params.t_span(end) 0.01 1]);
    %     xlabel('Time (days)');
...
for counter=1:length(switch_over_var)
    ...
    switch which_var
        ...
        case 2
            ...
            infile = 'SEIR_agedep_twodiseases_variationsymptomaticity_Wuhan_T5and8_mit.mat';
            load(strcat('./sim_data/',infile));
            cbf_colors = [133,192,249]/255; % light blue
            gamma_a2 = params.gamma_a;
            gamma_s2 = params.gamma_s;
    ...
    for count=1:8
        %         this_p = plot(params.t_span, results.ave_age_i_all,'Color',colors_rgb_vector,'LineWidth',2); hold on;
        this_p(count)=semilogy(params.t_span, results.y_traj(:,count)/data_Wuhan_Davies.age_distribution(count),'Color',colors_rgb_vector(count,:),'LineWidth',2); hold on;
    ...
    axis([0 params.t_span(end) 0.95 1]);
    %     xlabel('Time (days)');
    ylabel({'$S_n/A_n$'},'Interpreter','Latex');
    yticks([0.95 1]);
