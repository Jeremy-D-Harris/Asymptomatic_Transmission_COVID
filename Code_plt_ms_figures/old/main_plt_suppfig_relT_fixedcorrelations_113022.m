...
figure_name = 'Figsupp_fixedcorrelations_relevance_vs_relT_113022';
...
for which_file = 1:4
    ...
    for count=1:length(k_vector_relT)
        ...
        fixed_p(count) = params_collect(count).p;
    ...
    plt_fixed_p(which_file,:) = fixed_p;
    
    plt_diff_q_z_sameR0s(which_file,:) = diff_q_z_sameR0s;
    plt_change_prop_asymp_trans_sameR0s(which_file,:) = change_prop_asymp_trans_sameR0s;
    plt_diff_pt_p_sameR0s(which_file,:) = diff_pt_p_sameR0s;
    plt_change_prop_asymp_incidence_sameR0s(which_file,:) = change_prop_asymp_incidence_sameR0s;
...
for which_file = 1:4
    ...
    plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT(3)) = (plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT(3)-1)+plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT(3)+1))/2;
    
    plt_fixed_p(which_file,ind_k_vector_relT(3)) = (plt_fixed_p(which_file,ind_k_vector_relT(3)-1)+plt_fixed_p(which_file,ind_k_vector_relT(3)+1))/2;
...
%     xlim([5/8 8/5]);
    ...
    if which_file ==4
        ...
        legend_char1 = ['$\mathcal R_{0,a} = \mathcal R_{0,s}$ (fixed intrinsic proportion, $p$)'];
        legend_char2 = ['$\mathcal R_{0,a} = \mathcal R_{0,s}$'];
        legend_char3 = ['$\mathcal R_{0,a} = 4\,\mathcal R_{0,s}$ (fixed intrinsic proportion, $p$)'];
        legend_char4 = ['$\mathcal R_{0,a} = 4\,\mathcal R_{0,s}$'];
    ...
    subplot(2,2,3);
    if which_file == 1
        plot(k_vector_relT,plt_diff_pt_p_sameR0s(which_file,:),'-','Color',cbf_colors_vector(1,:),'LineWidth',2,'MarkerSize',18); hold on;
        plot(k_vector_relT(ind_k_vector_relT),plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT),'.','Color',cbf_colors_vector(1,:),'LineWidth',2,'MarkerSize',18); hold on;
    elseif which_file == 2
        plot(k_vector_relT,plt_diff_pt_p_sameR0s(which_file,:),'--','Color',cbf_colors_vector(1,:),'LineWidth',2,'MarkerSize',18); hold on;
        plot(k_vector_relT(ind_k_vector_relT),plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT),'.','Color',cbf_colors_vector(1,:),'LineWidth',2,'MarkerSize',18); hold on;
    elseif which_file == 3
        this_p = plot(k_vector_relT,plt_diff_pt_p_sameR0s(which_file,:),'Color',cbf_colors_vector(2,:),'LineWidth',3,'MarkerSize',18); hold on;
        this_p.Color(4) = 0.5;
%         this_p = plot(k_vector_relT(ind_k_vector_relT),plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT),'.','Color',[cbf_colors_vector(2,:),0.2],'MarkerSize',18); hold on;
        scatter(k_vector_relT(ind_k_vector_relT),plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT),'filled','SizeData',75,'MarkerEdgeColor',cbf_colors_vector(2,:),'MarkerFaceColor',cbf_colors_vector(2,:),'MarkerFaceAlpha', .5);
        
    elseif which_file == 4
        plot(k_vector_relT,plt_diff_pt_p_sameR0s(which_file,:),'--','Color',cbf_colors_vector(2,:),'LineWidth',2,'MarkerSize',18); hold on;
        plot(k_vector_relT(ind_k_vector_relT),plt_diff_pt_p_sameR0s(which_file,ind_k_vector_relT),'.','Color',cbf_colors_vector(2,:),'LineWidth',2,'MarkerSize',18); hold on;
    end
