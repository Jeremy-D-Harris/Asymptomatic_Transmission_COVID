...
for counter=1:3
    ...
    switch counter
        ...
        case 3
            ...
            load(strcat('./sim_data/',infile));
            cbf_colors = [0.5, 0.5, 0.5]; % dark blue
    ...
    this_q(counter) = semilogy(params.t_span, results.I_tot,'Color',cbf_colors,'LineWidth',2); hold on;
...
for counter=1:3
    ...
    this_q=plot(params.t_span, results.proportion_asymp_transmission,'Color',cbf_colors,'LineWidth',2); hold on;
    this_q.Color(4) = 1-0.18*(counter);
    ...
    this_q=plot(params.t_span, results.proportion_asymp_incidence,'Color',cbf_colors,'LineWidth',2); hold on;
    this_q.Color(4) = 1-0.18*(counter);
    axis([0 params.t_span(end) 0 1]);
    %     xlabel('Time (days)');
    ylabel({'Proportion'; 'Asymptomatic'; 'Incidence, $p_r(t)$'},'Interpreter','Latex');
    ...
    semilogy(params.t_span,ones(size(params.t_span)),'k','LineWidth',0.5); hold on;
    this_q=semilogy(params.t_span,results.Rt_fixedpropasymp,'Color',cbf_colors,'LineWidth',2); hold on;
    R_inf(counter) = results.Rt_fixedpropasymp(end);
    this_q.Color(4) = 1-0.18*(counter);
