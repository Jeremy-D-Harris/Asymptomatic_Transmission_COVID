...
%% simulate SEIR model with assortative mixing 
% according to p(a|a), p(a|s)
% same betas, beta_a = beta_s
...
if which_timescales==1
    ...
    % set beta_a = beta_s and r=0.14
    beta_a = 0.4840; beta_s = beta_a;
    
    p_aa = 0.7006; % contour data from S9
    p_as = 0.1996;
    ...
    if with_mitigation==0
        filename = 'SEIR_assortmixing_twodiseases_samebetas_071221_T5and5.mat';
    else
        filename = 'SEIR_assortmixing_twodiseases_samebetas_071221_T5and5_mit.mat';
    end
...
elseif which_timescales==2
    ...
    if with_mitigation==0
        filename = 'SEIR_assortmixing_twodiseases_samebetas_071221_T5and8.mat';
    else
        filename = 'SEIR_assortmixing_twodiseases_samebetas_071221_T5and8_mit.mat';
    end
...
subplot(4,1,1);
q = semilogy(params.t_span, I_tot,'Color',cbf_colors,'LineWidth',2); hold on;
axis([0 t_end 10^(-6) 1]);
...
r(1) = plot(params.t_span, proportion_asymp_incidence,'Color',cbf_colors,'LineWidth',2); hold on;
...
semilogy(params.t_span,Rt_assortmixing,'Color',cbf_colors,'LineWidth',2);
axis([0 t_end 10^-1 10^1]);
...
if save_ans==1
    ...
    folder_location = '../../Code_plot_ms_figures/sim_data/';
