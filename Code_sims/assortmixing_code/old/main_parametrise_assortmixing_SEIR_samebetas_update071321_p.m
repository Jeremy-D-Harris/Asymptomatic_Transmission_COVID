...
if which_timescales==1
    filename = 'parametrise_assortmixing_same_samebetas_SEIR_071221';
    ...
    filename = 'parametrise_assortmixing_diff_samebetas_SEIR_071221';
...
results.vary_p_aa_plt=vary_p_aa_plt;
results.vary_p_as_plt=vary_p_as_plt;
...
if plt_figure==1
    ...
    if which_timescales==1
        ...
        s1 = pcolor(results.vary_p_as_plt,results.vary_p_aa_plt,results.r_assortmixing_plt); hold on;
    else
        s1 = pcolor(results.vary_p_as_plt,results.vary_p_aa_plt,results.r_assortmixing_plt); hold on;
        ind_cont_r = contourc(results.r_assortmixing_plt,[level_r level_r]);
        contour_r = [results.vary_p_as_plt(round(ind_cont_r(1,2:end))); results.vary_p_aa_plt(round(ind_cont_r(2,2:end)))];
        
        ind_keep_cont_r = find(contour_r(2,:)>=level_prop_asymp_incidence & contour_r(2,:)< 1); % find P(a|a)>0.4
    ...
    % x variable: goes down the rows
    % y variable: goes across columns
    s2 = pcolor(results.vary_p_as_plt,results.vary_p_aa_plt,results.eigen_prop_asymp_incidence_plt); hold on;
